...
namespace APITest
{
    class Program
    {
        ...
        private static void Refresh(HttpClient _client)
        {
            ...
            var asUrl = $"http://localhost:56872/api/token/auth?grant_type=password&client_id={client_id}&client_secret={client_secret}&username={username}&password={password}";
            ...
            System.Threading.Thread.Sleep(TimeSpan.FromSeconds(10));//10s 过期

            Console.WriteLine("begin to request the resouce server");

            var rsUrl = "http://localhost:56966/api/values/1";
            ...
            if (rsMsg.StatusCode == HttpStatusCode.Unauthorized)
            {
                ...
                asUrl = $"http://localhost:56872/api/token/auth?grant_type=refresh_token&client_id={client_id}&client_secret={client_secret}&refresh_token={refresh_token}";
                ...
                HttpResponseMessage rsMsgNew = _client.GetAsync("http://localhost:56966/api/values/1").Result;
